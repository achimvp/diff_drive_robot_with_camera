#include "boxbot_arduino/arduino_comms.hpp"
...
void ArduinoComms::readServoValues(int &val_1, int &val_2)
{
    std::string response = sendMsg("t\r");
...
void ArduinoComms::readIMUValues(double acc[3], double gyro[3])
{
    ...
    acc[0] = std::atof(token_1.c_str());
    acc[1] = std::atof(token_2.c_str());
    acc[2] = std::atof(token_3.c_str());
    gyro[0] = std::atof(token_4.c_str());
    gyro[1] = std::atof(token_5.c_str());
    gyro[2] = std::atof(token_6.c_str());
}
