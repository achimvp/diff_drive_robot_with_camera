...
class RosControlNode(Node):
    def __init__(self):
        ...
        self.ctrl_pub = self.create_publisher(Float64MultiArray, "/rl/boxbot_action", 10)
        self._init_time = self.get_clock().now()
    
    def timer_callback(self):
        self.get_logger().info("Timer callback executed.")
        self.ctrl_pub.publish(Float64MultiArray(data=[1.0, 0.0, 1.0, 1.0]))  # Example command to move forward
